...
function deletevalue(event) {
  ...
  let url = Customers_URL + "?callback=ctrlq&name=" + name + "&id=" + id1 + "&action=delete";

  let request = jQuery.ajax({ crossDomain: true, url: url, method: "GET", dataType: "jsonp" });

  setTimeout(() => { location.href = "Data_Customers.html" }, 5000);
}
...
function insert_value() {
  ...
  let url = Customers_URL + "?callback=ctrlq&name=" + name + "&id=" + id1 + "&action=insert" + "&address=" + address + "&phone=" + phone
    + "&balance=" + balance;
...
function update_value() {
  ...
  let url = Customers_URL + "?callback=ctrlq&name=" + name + "&id=" + id1 + "&action=update" + "&address=" + address + "&phone=" + phone
    + "&balance=" + balance;
...
function CRUD(id) {
  ...
  if (id == "save") {
    ...
    insert_value(); setTimeout(() => { alert("تم حفظ البيانات"); location.href = "Data_Customers.html" }, 5000);
  } else {
    ...
    update_value(); setTimeout(() => { alert("تم تعديل البيانات"); location.href = "Data_Customers.html" }, 5000);
  }
}
