...
function Data(row) {
  ...
  for (let i = 1; i < row.length; i++) {
    colum = row[i];
    if (colum[4] === "الصندوق") {
      let obj = { nu: colum[0], date: colum[1], item: colum[2], statement: colum[3], name: "مقبوضات", amountdebit: colum[8], amountcredit: 0, balance: colum[13] }
      account.push(obj)

    } else if (colum[5] === "الصندوق") {
      let obj = { nu: colum[0], date: colum[1], item: colum[2], statement: colum[3], name: "مدفوعات", amountdebit: 0, amountcredit: colum[9], balance: colum[13] }
      account.push(obj)
    }
