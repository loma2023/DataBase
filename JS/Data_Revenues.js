...
function deletevalue(event) {
  ...
  let url = Revenues_URL + "?callback=ctrlq&name=" + name + "&id=" + id1 + "&action=delete";

  let request = jQuery.ajax({ crossDomain: true, url: url, method: "GET", dataType: "jsonp" });

  setTimeout(() => { location.href = "Data_Revenues.html" }, 5000);
}
...
function insert_value() {
  ...
  let url = Revenues_URL + "?callback=ctrlq&name=" + name + "&id=" + id1 + "&action=insert"
...
function update_value() {
  ...
  let url = Revenues_URL + "?callback=ctrlq&name=" + name + "&id=" + id1 + "&action=update"
...
function CRUD(id) {
  ...
  if (name == "") { alert("اكتب اسم الايراد"); return }
  ...
  if (id == "save") {
    ...
    insert_value(); setTimeout(() => { alert("تم حفظ البيانات"); location.href = "Data_Revenues.html" }, 5000);
  } else {
    ...
    update_value(); setTimeout(() => { alert("تم تعديل البيانات"); location.href = "Data_Revenues.html" }, 5000);
  }
}
