...
function Data(row) {
  ...
  for (let i = 1; i < row.length; i++) {
    colum = row[i];
    if (colum[4] === "الايرادات") {
      let obj = { nu: colum[0], date: colum[1], item: colum[2], statement: colum[3], name: colum[6], amountdebit: colum[8], amountcredit: 0, balance: colum[17] }
      account.push(obj)

    } else if (colum[5] === "الايرادات") {
      let obj = { nu: colum[0], date: colum[1], item: colum[2], statement: colum[3], name: colum[7], amountdebit: 0, amountcredit: colum[9], balance: colum[17] }
      account.push(obj)
    }
